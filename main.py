...
def create_model(emb_size, vocab_size, lstm_hidden_size, T, learning_rate=0.001):
    ...
    # filter_ = tf.nn.l2_normalize(tf.get_variable("letter_filter", shape=(1,T,1)))
    # # filter_ = tf.get_variable("letter_filter", shape=(1,T,1))
    # # embd_unrolled = tf.reshape(embedded, shape=(-1, T*emb_size))
    # embedded_pooled = tf.reduce_mean(embedded * filter_, axis=1)
    # hidden = tf.layers.dense(embedded_pooled, lstm_hidden_size)
...
with tf.Session() as sess:
    ...
    for e in range(epochs):
        for batch in batches:
            ...
            _, loss_val_train, train_class = sess.run([train_, loss_, classify_], {
                input_: names,
                labels_: labels
            })
